...
public class PalantiriModel
    ...
    private PalantiriManager palantiriManager;
        ...
        mServiceConnection = 
            new ServiceConnection(){
				...
				public void onServiceConnected(ComponentName name, IBinder service) {
					palantiriManager = PalantiriManager.Stub.asInterface(service);
					presenter.start();
				}
				@Override
				public void onServiceDisconnected(ComponentName name) {
					palantiriManager = null;
				}
    ...
    public void makePalantiri(int palantiriCount) {
        try {
            ...
            if (palantiriManager != null)
                ...
                palantiriManager.makePalantiri(palantiriCount);
            else 
    ...
    public Palantir acquirePalantir(long leaseDurationInMillis) {
        try {
            if (palantiriManager != null) {
                ...
                // proper code.
                final Palantir palantir = palantiriManager.acquire(leaseDurationInMillis, mLeaseCallback);
                ...
                // TODO - You fill in here.
                // create the record if not exist, or we update it
                if (mPalantirRecordMap.get(palantir.getId()) == null){
                    mPalantirRecordMap.put(palantir.getId(), new PalantirRecord(palantir.getGenerationCount()));
                } else {
                    mPalantirRecordMap.get(palantir.getId()).set(palantir.getGenerationCount());
                }
                /*
                Log.d(TAG, "Palantir "
                      + palantir.getId()
    ...
    public void releasePalantir(final Palantir palantir) {
        try {
            ...
            if (palantiriManager != null
                ...
                // expirations.
                // TODO - You fill in here.
            	// simply set it non-positive value to negate it
            	mPalantirRecordMap.get(palantir.getId()).set(-palantir.getGenerationCount());
    ...
    public long remainingTime(Palantir palantir) {
        try {
            if (palantiriManager != null) 
                ...
                // the proper call.
                return palantiriManager.remainingTime(palantir);
            else 
                Log.d(TAG, "palantiriManager was null.");
        } catch (RemoteException e) {
            e.printStackTrace();
