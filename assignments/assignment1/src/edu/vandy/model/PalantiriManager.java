...
public class PalantiriManager {
    ...
    public PalantiriManager(List<Palantir> palantiri,
    	...
    	mAvailablePalantiri = semaphore;
    	mPalantiriMap = new HashMap<>();

    	for(int i=0;i<palantiri.size();i++){
    		mPalantiriMap.put(palantiri.get(i), mNotInUse);
    	}
    }
    ...
    public Palantir acquire(long leaseDurationInMillis) {
        // @@ TODO -- you fill in here.
    	Palantir palantiri = null;
		mAvailablePalantiri.acquireUninterruptibly();
		synchronized(this){
	    	Iterator it = mPalantiriMap.entrySet().iterator();
	    	while(it.hasNext()){
	    		Entry entry = (java.util.Map.Entry)it.next();
	    		LeaseState ls = (LeaseState) entry.getValue();
	    		if(ls.equals(mNotInUse)){
	    			palantiri = (Palantir) entry.getKey();
    ...
    public void release(final Palantir palantir) {
        // @@ This code is "brittle", i.e., what happens if the palantir is null?

        // @@ TODO -- you fill in here.
    	if(palantir!=null){
	    	mAvailablePalantiri.release();
	    	synchronized(this){
	    		mPalantiriMap.get(palantir).close();
	    		mPalantiriMap.put(palantir, mNotInUse);
	    	}
    	}
    ...
    public long remainingTime(Palantir palantir) {
        // @@ This code is "brittle", i.e., what happens if the palantir is null?
        // @@ Yikes, there are race conditions in this code!
