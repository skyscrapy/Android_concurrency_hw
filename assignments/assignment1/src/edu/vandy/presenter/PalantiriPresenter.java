...
public class PalantiriPresenter 
    ...
    private void beginBeingsThreads(int beingCount) {
    	...
    	BeingThread newThread;
    	for(int i=0;i<beingCount;i++){
    		newThread = new BeingThread(new BeingRunnable(i, PalantiriPresenter.this), i, PalantiriPresenter.this);
    		mBeingsThreads.add(newThread);
    	}
<<<<<<< HEAD
    	for(BeingThread bThread:mBeingsThreads)
    		bThread.start();
=======

        // @@ Use a for-each loop iterator here instead of a for loop with a loop counter.

    	for(int i=0;i<beingCount;i++)
    		mBeingsThreads.get(i).start();
        
>>>>>>> 34eb75f83b0e94ccd2fbbaeffab9f88801f481a2
    }
    ...
    private void waitForBeingsThreads() {
        ...
        // finish and then informs the View layer that the simulation
        // is done.
<<<<<<< HEAD
=======
    // @@ This implementation is extremely inefficient since it's polling!
    	Thread doneThread = new Thread(new Runnable(){
    		public void run(){
    				while(BeingThread.isShutdown()){
			    		mView.get().done();
			    		break;
	    			}	
    			}
...
>>>>>>> 34eb75f83b0e94ccd2fbbaeffab9f88801f481a2
    	...
    	final Thread doneThread = new Thread(new Runnable(){
    		public void run(){	
    			for(BeingThread bThread:mBeingsThreads){
	    			try {
	    				bThread.join();
