...
public class PalantiriManager {
    ...
    public Palantir acquire(long leaseDurationInMillis) {
    	...
    	// acquire the writelock so that we can ensure only one thread get palantiri
    	// if count is greater than 0, get the palantiri directly and decrement the count
    	// otherwise wait at waitnode and get palantiri when available
    	// finally release the lock
    	mLock.writeLock().lock();
    	try{
    		if(mAvailablePalantiri>0){
    			mAvailablePalantiri--;
    			return getPalantir(leaseDurationInMillis);
    		}else{
    			waitnode = new WaitNode();
    			mWaitQueue.add(waitnode);
    			return waitnode.getPalantirWhenAvailable(leaseDurationInMillis);
    		}		
	    }finally{
    ...
    public void release(final Palantir palantir) {
    	...
    	WaitNode waitnode = null;
    	// Acquire the writelock so as to ensure only one thread to put back the palantiri.
    	// If the queue isn't empty and there's no palantiri available, then put the palantiri back
    	// and signal the waitnode as FIFO order in the wait queue.
    	// Otherwise simply put back the palantiri. Finally release the lock
    	mLock.writeLock().lock();
    	try{
			if(mAvailablePalantiri==0 && !mWaitQueue.isEmpty()){
				waitnode = mWaitQueue.poll();
				putPalantir(palantir);
				waitnode.wakeupWaitingThread();
			}else{
				putPalantir(palantir);
				mAvailablePalantiri++;
			}
    ...
    public long remainingTime(Palantir palantir) {
    	...
    	if(palantir!=null){
    		// Acquire readlock so that all threads can read the data
			mLock.readLock().lock();
			try{
    ...
    private class WaitNode {
        ...
        public WaitNode() {
            mReleased = false;
            mCondition = mLock.writeLock().newCondition();
    ...
    public long availablePalantiri() {
    	...
    	long num = 0;
    	// Acquire the readlock so as to read the count
    	mLock.readLock().lock();
    	try{
    		num = mAvailablePalantiri;
    	} finally{
    		mLock.readLock().unlock();
    	}
    	return num;
